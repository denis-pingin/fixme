...
- `brokerChildAttention` uses `status: "waitingForUser"` while showing the child prompt. It records the answer through attention broker APIs, never interprets the answer, and never creates a new task or re-passes the routed PR fix text. After the answer exists, it persists `activeChild.resumeDispatch` with an idempotency key derived from `parentRunId + activeBatchIndex + activeChild.attentionId + answeredAt`, then resumes the same child `fixme-task` with the active child's `resumeRef`, `--answer-attention <attentionId>`, the same child `statusId`, and the same transport used for the original child dispatch. If the answer already exists but `resumeDispatch` is missing, retry writes it and resumes the child. If `resumeDispatch` already exists, retry returns the existing dispatch evidence. It returns to `awaitFixmeTask` only after the resume dispatch has started or been idempotently acknowledged.
