...
- **Playwright MCP (`@playwright/mcp`)**: Browser automation via accessibility tree snapshots (text-based, no vision model needed). Registered once via `claude mcp add`.
...
5. **Browser layer** (Playwright MCP): Snapshot-driven interaction for reproduction and verification. Accessibility tree for decisions, screenshots for evidence.
...
**Rationale:** The ARCHITECTURE research identifies browser reproduction as the highest-risk component that blocks both fixing and verification. Build it early, validate Playwright MCP patterns work reliably. The reproducer agent shares patterns with the verifier agent -- get them right once.
...
**Uses:** Playwright MCP (`browser_navigate`, `browser_snapshot`, `browser_click`, `browser_screenshot`).
...
- **Phase 3 before 4-5:** Browser reproduction is the highest-risk technical component (Playwright MCP integration patterns). It blocks both fixing (needs repro evidence) and verification (reuses repro patterns). Build and validate it early.
...
- **Phase 3 (Browser Reproduction):** Playwright MCP accessibility tree limitations are partially documented (GitHub issue #514). Shadow DOM behavior, multi-tab management, and interaction reliability need hands-on validation. Run `/gsd:research-phase` before planning.
...
| Pitfalls | HIGH | 8 pitfalls identified from Anthropic engineering docs, Claude Code official docs, Playwright MCP issues, and community incident reports. Each has prevention strategy and phase mapping. |

**Overall confidence:** HIGH

All four research streams converge on the same architectural conclusions. The GSD reference is a locally inspectable, working system -- not documentation that might be outdated. The skill/subagent/MCP stack is first-party Anthropic technology with official documentation. Playwright MCP is maintained by Microsoft with active development.

### Gaps to Address

- **Playwright MCP tab management under concurrent agents:** The research identifies that subagents may share a browser process. Exact behavior when intake (background) and fixer (foreground) both need browser access is untested. Validate in Phase 3.
