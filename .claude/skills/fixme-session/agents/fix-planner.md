...
tools: Read, Write, Glob, Grep, Bash(node ~/.claude/skills/fixme-session/scripts/fixme-tools.cjs *)
model: inherit
---
...
```bash
node ~/.claude/skills/fixme-session/scripts/fixme-tools.cjs ticket transition <ticket-folder>/ticket.md planning
```

**Retry** (ticket is in `verifying` state):
```bash
node ~/.claude/skills/fixme-session/scripts/fixme-tools.cjs ticket transition <ticket-folder>/ticket.md planning --reason "<transition reason from dispatch prompt>"
```
