...
Runtime wait semantics: parents that wait on a dispatched child treat `run status.updatedAt` as the last status write, not a heartbeat. On a runtime wait watchdog timeout the parent calls `lifecycle dispatch reconcile-wait` and branches only on its returned transition (`runtimeWaitTimedOut`, `terminalEvent`, `attention`, `stalledOwner`, `dispatchFailure`). An old `updatedAt` with an attached active runtime and no terminal/attention evidence is `owned, wait timed out, liveness unknown`, not proof of live host-runtime work.
