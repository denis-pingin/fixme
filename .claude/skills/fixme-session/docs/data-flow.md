...
Runtime wait semantics: parents that wait on a dispatched child treat `run status.updatedAt` as the last status write, not a heartbeat. Only `run status.workerHeartbeat.observedAt` is child-owned liveness. On a runtime wait watchdog timeout the parent calls `lifecycle dispatch probe` and branches only on its returned transition (`continueWait`, `terminalEvent`, `attention`, `stalledOwner`, `dispatchFailure`). `continueWait` re-arms the same runtime wait action. An old or missing `workerHeartbeat` with an attached active runtime and no terminal/attention evidence is not proof of dead host-runtime work.
