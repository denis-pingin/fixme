...
**Watchdog wait policy:** wait on a dispatched child by transport. For an `agent`/`background` child, write one durable wait marker and then block silently on the runtime wait primitive with a 5-minute watchdog (`wait_agent({ targets: [id], timeout_ms: 300000 })` for Codex agents). On watchdog timeout, call exactly:

```bash
node ~/.claude/skills/fixme-tools/scripts/fixme-tools.cjs lifecycle dispatch probe --fixme-dir <fixme-dir> --dispatch-id <dispatch-id> --status-id <status-id> --data '{"parentStatePath":"<absolute-parent-state-path>","waitActionId":"<wait-action-id>","watchdogMs":300000,"probeReason":"waitWatchdogTimeout"}'
```

Then branch only on the returned `transition`; `runtimeWaitTimedOut` with `reason: "runtimeLivenessUnknown"` stops the current wait path and must be reported instead of silently waiting again, while `stalledOwner` runs `lifecycle dispatch stalled-owner recover` with the returned recovery data and then either executes the returned owner resume runtime action or treats `ownerStoppedBeforeDispatchCompletion` as dispatch failure. Do not hand-roll `run status` age thresholds. `run status.updatedAt` is the last status write, not a heartbeat; only `run status.workerHeartbeat.observedAt` is child-owned liveness detail. An old or missing heartbeat with an attached active runtime and no terminal/attention evidence is host-runtime liveness unknown, not proof of live work. For an `inline-skill` child, take the synchronous return; do not run a synthetic polling loop. For a `direct` user-facing turn, wait through the normal user turn. "Polling" means watchdog-timeout fallback or explicit status handling, not normal waiting. Durable attention semantics are unchanged.
...
node ~/.claude/skills/fixme-tools/scripts/fixme-tools.cjs lifecycle attention broker resume --fixme-dir <fixme-dir> --parent-run-id <activeParentRunId> --status-id <activeRunStatusId> --attention-id <attention-id> --data '{"answer":"<raw user answer or broker-show answer.answer>","answeredBy":"user","answerKind":"decision"}'
node ~/.claude/skills/fixme-tools/scripts/fixme-tools.cjs lifecycle attention broker resume --fixme-dir <fixme-dir> --parent-run-id <activeParentRunId> --status-id <activeRunStatusId> --attention-id <attention-id> --data '{"answer":"<raw user clarification question>","answeredBy":"user","answerKind":"clarificationRequest"}'
node ~/.claude/skills/fixme-tools/scripts/fixme-tools.cjs lifecycle attention broker acknowledge-resume --fixme-dir <fixme-dir> --parent-run-id <activeParentRunId> --status-id <activeRunStatusId> --attention-id <attention-id> --data-stdin <<'JSON'
<acknowledgeResumeTemplate.data plus runtimeHandle only when returned by runtime-action observe>
JSON
```

Call `lifecycle attention broker show`, read the returned `promptMarkdown` and `renderContract`, and present the prompt according to the Boundary Delivery Contract in `fixme-howto-present-decisions`. If the user response is a decision answer, run `node ~/.claude/skills/fixme-tools/scripts/fixme-tools.cjs lifecycle attention broker resume --fixme-dir <fixme-dir> --parent-run-id <activeParentRunId> --status-id <activeRunStatusId> --attention-id <attention-id> --data '{"answer":"<raw user answer>","answeredBy":"user","answerKind":"decision"}'`. If the user response is a clarifying question, run `node ~/.claude/skills/fixme-tools/scripts/fixme-tools.cjs lifecycle attention broker resume --fixme-dir <fixme-dir> --parent-run-id <activeParentRunId> --status-id <activeRunStatusId> --attention-id <attention-id> --data '{"answer":"<raw user clarification question>","answeredBy":"user","answerKind":"clarificationRequest"}'`. Execute exactly the returned `runtimeAction`, then call `lifecycle runtime-action observe` with evidence for that `actionId`; repeat while lifecycle returns `status: "requiresRuntimeAction"`. After broker resume returns, execute exactly the returned `runtimeAction` and observe it through `lifecycle runtime-action observe`. Then copy `acknowledgeResumeTemplate.data` exactly, add only the observed `runtimeHandle` when the runtime action returned one, and run the documented `lifecycle attention broker acknowledge-resume --data-stdin` command above. Do not compose `--resume <active_task> --answer-attention <attention-id>` by hand. The status id is context, not a command-line flag. Do not persist any task-owned decision; the background `fixme-task` resumes and writes decisions itself. Do not duplicate the Boundary Delivery Contract's normative prose here.

Use the `answer` object returned by broker show unchanged when `status` is `answered`.
...
4. **Determine pipeline:**
   Read `<fixme-dir>/config.json` to determine which workflow to use for this ticket. Default: `"bugfix"` for bug fix sessions (investigate -> research -> plan -> implement -> verify). Store the selected workflow name for both fixme-task dispatch and the pre-pipeline ticket transition command.
...
5. **Optionally dispatch pre-pipeline phases (synchronous):**
   ...
   **Before dispatching, transition the ticket to investigate:**
   Invoke fixme-tickets: `ticket transition <ticket-folder>/ticket.md investigate --pipeline <pipeline name from step 4>`
...
3. **Show active task:** If `active_task` is set, indicate which ticket has a background fixme-task running. If `activeRunStatusId` is also set, read liveness with:
   ...
   Show the active agent, state, checkpoint, current command, and `updatedAt`. `updatedAt` is the last status write, not a heartbeat; do not infer staleness or failure from an old `updatedAt` alone. If `currentCommand` is `attention:<attention-id>`, follow Attention ownership before showing any coarse status response. If `run status` fails, print a warning with `activeRunStatusId` and continue showing ticket state.
