...
Parent-facing brokers use `lifecycle attention broker show` and `lifecycle attention broker answer`; `run attention answer` and `run attention clear` are owner/internal APIs.
...
node ~/.claude/skills/fixme-tools/scripts/fixme-tools.cjs task init --task <task.md> --pipeline-resolution-file <absolute-json-file> --project-root <project-root>
