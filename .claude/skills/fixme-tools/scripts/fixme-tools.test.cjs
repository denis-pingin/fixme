...
test('fixme-pr-comments skill: triages comments by risk, complexity, confidence, and route', () => {
  ...
  assert(skill.includes('Start with the outcome, not the ledger.'), 'presentation should start with the reader outcome');
  assert(skill.includes('**Recommended next action**:'), 'presentation should include a clear recommendation near the top');
  assert(skill.includes('### Highest Priority First'), 'presentation should prioritize important items before accounting');
  ...
  assert(skill.includes('Do not start the report with a markdown table.'), 'presentation should forbid table-first output');
  ...
  assert(skill.includes('Expand full evidence cards for BLOCKER, MAJOR, FIX_UNCLEAR, ASK_USER, LOW confidence, or PLAN_REQUIRED groups.'), 'presentation should avoid expanding every low-risk item');
...
test('fixme decision presentation skill: uses visually scannable cards', () => {
  ...
  assert(skill.includes('- **Why now**: {what this decision unlocks or prevents}'), 'at-a-glance block should include urgency');
