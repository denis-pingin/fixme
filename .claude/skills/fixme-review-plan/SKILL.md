...
2. **Can this state actually happen?** Verify reachability from the codebase, task code map, plan, spec, tests, API contracts, schemas, caller guards, or state machine.
...
- **Supported** - the state is reachable and the product/domain/API contract says it must work. Promote a finding only if the plan fails to support it.
- **Unsupported but reachable** - the state can happen, but should be rejected or constrained earlier. Promote a finding only when the plan lacks that boundary behavior; the suggestion must be fail-fast or make-impossible, not broad downstream support.
- **Impossible by construction** - the current types, schema, caller guards, or state machine make the state unreachable. Drop the candidate silently unless the plan weakens those guarantees.
