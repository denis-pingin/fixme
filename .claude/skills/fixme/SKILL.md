...
- Wait by transport: for an `agent`/`background` child, block silently on the runtime wait primitive with a 5-minute watchdog (`wait_agent({ targets: [id], timeout_ms: 300000 })` for Codex agents); on watchdog timeout call `lifecycle dispatch reconcile-wait` and branch only on the returned `transition` rather than hand-rolling `run status` age thresholds (`run status.updatedAt` is the last status write, not a heartbeat); for an `inline-skill` child, take the synchronous return without a synthetic polling loop; for a `direct` user-facing turn, wait through the normal user turn. "Polling" means watchdog-timeout fallback or explicit status handling, not normal waiting; durable attention semantics are unchanged.
...
- If the current context is brokering a pending `fixme-task` attention prompt or attention answer, call `lifecycle attention broker show`, read the returned `promptMarkdown` and `renderContract`, and present the prompt according to the Boundary Delivery Contract in `fixme-howto-present-decisions`. If the user gives a decision answer, call `lifecycle attention broker resume` with `{ "answer": "<user answer>", "answeredBy": "user", "answerKind": "decision" }`. If the user asks a clarifying question, call the same command with `{ "answer": "<user answer>", "answeredBy": "user", "answerKind": "clarificationRequest" }`. Launch only the returned `resume.message`, then copy the `acknowledgeResumeTemplate.data` from the broker resume output, add only the runtime-derived launch evidence (`transport`, `runtime`, optional `runtimeHandle`), and call `lifecycle attention broker acknowledge-resume` with that payload. If broker show returns `status: "answered"`, do not present the prompt again; call broker resume with the returned `answer`, launch only the returned `resume.message`, then acknowledge-resume with the same post-launch evidence shape. If the resumed `fixme-task` returns another `FIXME_ATTENTION_REQUIRED`, broker that new prompt the same way. Do not route the message as a fresh saved task or one-off implementation workflow, and do not duplicate the Boundary Delivery Contract's normative prose in this router.
